...
function JobLevel() {
  ...
  const handleSelectAll = async (e) => {
    ...
    var arr = [];
    if (!isCheckedAll) {
      for (var x = 0;x < joblevel.length;x++) {
        arr.push(joblevel[x].id);
      }
    }
    setIsCheck(arr);
  };
  ...
  const exportExcel = async () => {
    if (joblevel.length > 0) {
      ...
      XLSX.writeFile(wb, "Job Level.xlsx");
            ...
            <Button
              ...
              onClick={async () => {
                for (var x = 0;x < isCheck.length;x++) {
                  await DeleteJobLevel(isCheck[x]);
                }
                  ...
                  <tr>
                    <td className="align-middle">
                      <input
                        ...
                        checked={isCheck.includes(val.id) ?? false}
